...
export const postgresqlRepositoryMigrations = Object.freeze([
  ...
  Object.freeze({
    ...
    description: "Add durable queue visibility metadata for WorkerJobRepositoryPort reservations.",
    statements: Object.freeze([
  ...
  Object.freeze({
    ...
    description: "Create PostgreSQL durable storage for AsyncEventLogPort.",
    statements: Object.freeze([
      `CREATE TABLE IF NOT EXISTS omniwa_event_log (
      ...
      `CREATE TABLE IF NOT EXISTS omniwa_event_outbox (
        outbox_id text PRIMARY KEY,
        event_id text NOT NULL UNIQUE REFERENCES omniwa_event_log(id) ON DELETE CASCADE,
        cursor text NOT NULL,
        status text NOT NULL,
        created_at timestamptz NOT NULL,
        published_at timestamptz NULL
      )`,
      "CREATE INDEX IF NOT EXISTS omniwa_event_log_type_idx ON omniwa_event_log (event_type)",
      "CREATE INDEX IF NOT EXISTS omniwa_event_log_timestamp_idx ON omniwa_event_log (event_timestamp)",
      "CREATE INDEX IF NOT EXISTS omniwa_event_log_resource_ref_idx ON omniwa_event_log (resource_ref)",
      "CREATE INDEX IF NOT EXISTS omniwa_event_log_correlation_id_idx ON omniwa_event_log (correlation_id)",
      "CREATE INDEX IF NOT EXISTS omniwa_event_outbox_status_idx ON omniwa_event_outbox (status)",
  ...
  implements WorkerJobRepositoryPort
{
  ...
  async reserveNextVisibleWorkerJob(
    ...
    if (this.transactionConnection === undefined) {
      throw new TypeError(
        "PostgreSQL WorkerJob atomic reservation requires a transaction connection.",
      );
    }
  ...
  async setWorkerJobVisibleAt(jobId: JobId, visibleAtEpochMilliseconds: number): Promise<void> {
    await this.ensureReady();
