...
bridge proof refs, queue runtime proof refs, observability signal proof refs, and backup/restore
...
or raw runtime logs, and keeps status failed unless every required runtime, dependency, bridge,
queue runtime, observability, and backup/restore proof is complete. It does not replace the proof
state updates required in
...
Queue runtime proof must include safe refs for durable queue profile selection, atomic reservation,
retry recovery, dead-letter behavior, and expired lease recovery.
