...
| P0-08 | Partial                 | Rate limiter foundation exists and API runtime composition can now opt in through `OMNIWA_API_RATE_LIMIT_MAX_REQUESTS`, `OMNIWA_API_RATE_LIMIT_WINDOW_MS`, and endpoint-class override env vars. Rate-limit snapshots can now export low-cardinality API metric points. The rate-limit port is async-compatible and has shared counter-store, Redis script-store, and runtime injection foundations with hashed Redis keys. Production API composition now rejects missing rate-limit configuration, the in-memory backend, and Redis backend selection without an injected script client before the remaining production-adapter fail-safe. Production Redis client adapter/dependency selection is gated by proposed ADR-0008, and guardrail integration still needs hardening.                                                                                                                                                                  |
...
- Rate limits exist by key, instance, and endpoint class; the current runtime-wired limiter is
  ...
  production. The async store-backed limiter, Redis script store, and runtime injection now exist;
  production API composition requires Redis backend configuration with an injected script client, and
  the concrete production Redis client adapter/dependency remains required before promotion after
  ADR-0008 is accepted or revised.
