...
- Denied security decisions produce safe evidence; the current runtime-wired sinks are opt-in
  ...
  provides `AuditRecordRepositoryPort`. Production database credentials and deployment profile
  validation are still required before production promotion.
