...
| P0-07 | Partial                 | API key auth/lifecycle foundations exist, and API runtime can now compose from `OMNIWA_API_KEY_HASH` or `OMNIWA_API_KEY_LIFECYCLE_STORE_PATH` without retaining plaintext key config. Admin rotation/revocation runtime paths, least privilege, and audit hardening remain open. |
