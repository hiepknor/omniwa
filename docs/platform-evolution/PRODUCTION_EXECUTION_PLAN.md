...
| P0-02 | Partially closed        | API runtime composition exists for local/dev and Docker local. Production API composition now validates PostgreSQL, Redis-backed rate limiting, AuditRecord-backed security audit, repository-backed ownership, durable API queue profile configuration, and an API request metric recorder supplied by `OMNIWA_API_METRICS_JSONL_PATH` or injection. Target-environment startup verification still needs completion.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| P0-06 | Partial                 | Durable WorkerJob-backed queue provider exists behind `QueueProviderPort`; API runtime composition can opt into it with `OMNIWA_API_QUEUE_PROFILE=durable-worker-job` and production API runtime validation requires that queue profile. Cross-process atomic leasing, worker/provider process proof, and final production queue semantics remain open.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| P0-08 | Partial                 | Rate limiter foundation exists and API runtime composition can now opt in through `OMNIWA_API_RATE_LIMIT_MAX_REQUESTS`, `OMNIWA_API_RATE_LIMIT_WINDOW_MS`, and endpoint-class override env vars. Rate-limit snapshots can now export low-cardinality API metric points. The rate-limit port is async-compatible and has shared counter-store, Redis script-store, runtime injection, and concrete API-runtime Redis client adapter foundations with hashed Redis keys. Production API composition now rejects missing rate-limit configuration, the in-memory backend, and Redis backend selection without `OMNIWA_API_RATE_LIMIT_REDIS_URL` or an injected script client before accepting production composition. Guardrail integration still needs hardening.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| P0-09 | Partial                 | Resource ownership checks exist for current surfaces. API runtime composition can opt in to in-memory denied-decision evidence with `OMNIWA_API_SECURITY_AUDIT_IN_MEMORY=true`, durable JSON denied-decision evidence with `OMNIWA_API_SECURITY_AUDIT_LOG_PATH`, approved domain AuditRecord evidence with `OMNIWA_API_SECURITY_AUDIT_RECORDS=true`, and repository-backed ownership resolution with `OMNIWA_API_RESOURCE_OWNERSHIP_REPOSITORY=true` for session/message/attached-media/chat/contact/label/group/job resources that carry explicit or safely derivable instance ownership. Targetless global resources such as webhooks, deliveries, events, jobs, metrics, audit records, API keys, settings, and provider status now fail closed for instance-scoped credentials unless a future resource model supplies an explicit owner or filtered read model. API production runtime composition now rejects local PostgreSQL hosts, known development database credentials, unsafe/missing rate-limit posture, non-AuditRecord security-audit posture, missing repository-backed ownership resolution, missing durable queue profile, and missing API metric recorder before accepting production composition. Future owner modeling for currently global resources, deployment profile validation, and regression depth need hardening.                                                   |
...
| P0-12 | Closed for current cut  | Webhook dispatcher runtime exists with durable restart recovery, retry/dead-letter tests, persisted `WebhookDelivery` delivered/retrying/dead-letter status transitions, a durable worker-job queue profile, HMAC/timestamp signing, replay verification, metrics, audit evidence, a `FetchWebhookHttpGateway` foundation for safe timeout/network failure mapping, opt-in local runtime fetch gateway wiring that requires a signing secret name, public controlled webhook delivery retry for eligible pending/retrying deliveries, public controlled redrive for eligible dead-lettered deliveries, and selected bulk redrive for dead-lettered deliveries through a safe operation intent, and dead-letter filtered operator listing with safe remediation reason/category filters in the public client contract. Production profile composition now fails closed unless PostgreSQL repositories, the durable worker-job queue profile, fetch gateway, configured signing secret value, metric recorder, and webhook dispatch audit sink are present, runtime composition can satisfy metric/audit observability through distinct writable JSONL sink paths, and `pnpm test:postgres` covers a production-profile webhook dispatcher dispatch path. Richer dead-letter workflows move to P1-04, richer exporters and JSONL rotation move to P0-13, and broader E2E proof moves to P0-15/N11.7. |
| P0-13 | Gate foundation closed  | Logging/health/readiness foundations exist, and the explicit `pnpm observability:check` root gate now verifies observability readiness evidence, metric catalog coverage, alert definitions, dependency readiness behavior, metrics runtime smoke, health runtime smoke, and release-readiness wiring. API production composition now has a concrete request-latency metrics adapter boundary and fails closed unless a writable JSONL metrics path or injected recorder is configured. Target-environment dashboards, alert routing, exporter operations, JSONL rotation/exporter decisions, and sustained SLO monitoring remain required before a broad production-ready claim.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| P0-14 | Gate foundation closed  | Deterministic backup/restore recovery drill, recovery validation runner, runbook, release evidence, and the explicit `pnpm recovery:check` root gate exist. Target-environment backup automation and restore drill evidence remain required before a broad production-ready claim.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| P0-15 | Gate foundation closed  | Broad unit/integration/regression gates exist, and the explicit `pnpm e2e:check` root gate now requires deterministic REST regression plus local vertical-slice runtime proof across Application, durable JSON state, queue, worker, provider fake socket, and EventLog safety. Target-environment E2E smoke tests remain required before a broad production-ready claim.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| P0-16 | Gate foundation closed  | Security tests exist for current controls, and the explicit `pnpm security:check` root gate now requires API auth, API-key lifecycle, rate limiting, security audit evidence, resource ownership, webhook signing/replay, redaction, object-path secrecy, and Baileys auth-state safety coverage. External penetration testing and deployment-specific security review remain required before a broad production-ready claim.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| P0-17 | Gate foundation closed  | Load baseline tests exist, and the explicit `pnpm performance:check` root gate now verifies performance readiness evidence, delegates to `pnpm load:check`, and requires the load baseline and production cut checker tests. The production cut checker now requires explicit target-environment, production-load, and SLO-evidence proof states and blocks `PRODUCTION_READY` when any remain unproven. A dedicated `pnpm target-env:check` gate and `docs/reviews/TARGET_ENVIRONMENT_VALIDATION.md` evidence file now track the remaining target-environment proof without pretending it is complete. `pnpm target-env:smoke` provides an optional deployed-API smoke command for `/v1/health`, `/v1/health/readiness`, and `/v1/instances`, and `pnpm target-env:load` provides an optional bounded deployed-API load command for the approved public GET endpoint set. External dependency capacity evidence and sustained SLO observation remain required before a broad production-ready claim.                                                                                                                                                                                                                                                                                                                                                                                              |
...
N11.1 Production Queue Foundation (done)
  ...
  -> N11.5 Authorization and Rate Limits (done)
  -> N11.6 Webhook Reliability (done)
...
- Rate limits exist by key, instance, and endpoint class; the current runtime-wired limiter is
  opt-in in-memory for local use and backed by the approved Redis script client adapter when
  `OMNIWA_API_RATE_LIMIT_BACKEND=redis` is configured. The async store-backed limiter, Redis script
  store, runtime injection, and concrete API-runtime Redis client adapter now exist; production API
  composition requires Redis backend configuration with `OMNIWA_API_RATE_LIMIT_REDIS_URL` or an
  injected script client before promotion.
- Resource IDs resolve to ownership before authorization decisions; the current repository-backed
  resolver covers instance-owned resource aggregates, labels, attached media through message
  ownership, and jobs with safe instance metadata, then fails closed for unsupported resources when
  enabled.
- Denied security decisions produce safe evidence; the current runtime-wired sinks are opt-in
  in-memory, durable JSON, or domain AuditRecord-backed where the selected repository profile
  provides `AuditRecordRepositoryPort`. API production runtime composition validates PostgreSQL
  credential posture, Redis-backed rate-limit posture, AuditRecord-backed security-audit posture,
  repository-backed resource-ownership posture, durable queue profile, and API metric recorder
  presence before accepting production composition; broader deployment profile validation is still
...
- Ownership resolver for instance, message, attached-media, chat, contact, label, group, and job
  resources with explicit or safely derivable owners.
...
- `pnpm observability:check` passes and is included in `pnpm check`.
