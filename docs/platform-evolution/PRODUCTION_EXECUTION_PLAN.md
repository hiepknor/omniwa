...
| P0-12 | Partial                 | Webhook dispatcher runtime exists with durable restart recovery, retry/dead-letter tests, persisted `WebhookDelivery` delivered/retrying/dead-letter status transitions, a durable worker-job queue profile, HMAC/timestamp signing, replay verification, metrics, audit evidence, a `FetchWebhookHttpGateway` foundation for safe timeout/network failure mapping, opt-in local runtime fetch gateway wiring that requires a signing secret name, public controlled webhook delivery retry for eligible pending/retrying deliveries, public controlled redrive for eligible dead-lettered deliveries, and selected bulk redrive for dead-lettered deliveries through a safe operation intent, and dead-letter filtered operator listing with safe remediation reason/category filters in the public client contract. Production profile composition now fails closed unless PostgreSQL repositories, the durable worker-job queue profile, fetch gateway, configured signing secret value, metric recorder, and webhook dispatch audit sink are present, runtime composition can satisfy metric/audit observability through distinct JSONL sink paths, and `pnpm test:postgres` covers a production-profile webhook dispatcher dispatch path. Richer dead-letter operations, broader production E2E validation, and target-environment JSONL writability/rotation validation remain open. |
...
N11.1 Production Queue Foundation (done)
  ...
  -> N11.6 Webhook Reliability (current)
