...
| P0-08 | Partial                 | Rate limiter foundation exists and API runtime composition can now opt in through `OMNIWA_API_RATE_LIMIT_MAX_REQUESTS`, `OMNIWA_API_RATE_LIMIT_WINDOW_MS`, and endpoint-class override env vars. Rate-limit snapshots can now export low-cardinality API metric points. The rate-limit port is async-compatible and has shared counter-store plus Redis script-store foundations with hashed Redis keys. Production Redis client binding and guardrail integration need hardening.                                                                                                                                                                                                                                                                                                                                            |
...
- Rate limits exist by key, instance, and endpoint class; the current runtime-wired limiter is
  ...
  production. The async store-backed limiter and Redis script store now exist; the production Redis
  client binding remains required before promotion.
