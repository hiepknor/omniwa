...
| P0-07 | Partial                 | API key auth/lifecycle foundations exist, and API runtime can now compose from `OMNIWA_API_KEY_HASH` without retaining plaintext key config. Durable lifecycle wiring, rotation/revocation runtime paths, least privilege, and audit hardening remain open. |
