...
- Denied security decisions produce safe evidence; the current runtime-wired sinks are opt-in
  ...
  credential posture before the remaining production-adapter fail-safe; broader deployment profile
  validation is still required before production promotion.
