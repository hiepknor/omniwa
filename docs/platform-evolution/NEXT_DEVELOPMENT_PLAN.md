...
The queue observability hardening slice adds cataloged `queue.backlog.depth` and
`queue.backlog.oldest_pending_age` metrics from the durable WorkerJob queue provider. The metrics
use only the approved low-cardinality `work_type` label and are now covered by the observability
catalog, dashboard routing, metrics runtime smoke path, and durable queue provider tests.
...
event-stream proof refs for durable EventLog backend selection, replay cursor, expired cursor
handling, SSE cursor resume, public event envelope checks,
...
queue runtime proof, event-stream proof, credential-boundary proof, or observability signal refs
that still contain `pending` keep runtime evidence failed.
...
If a future `TARGET_ENVIRONMENT_VALIDATION.md` review claims `PROVEN`, `pnpm target-env:check` now
also requires `OMNIWA_TARGET_ENV_EVIDENCE_BUNDLE_PATH` to provide the matching sanitized evidence
bundle artifact, so a markdown-only proof promotion cannot pass the local release gate.
