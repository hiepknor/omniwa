...
| Runtime apps                                                                                     | Frozen                      | API, worker, webhook dispatcher, provider runtime, background, scheduler, health, metrics, and projection-builder apps exist | `apps/*/src`, runtime composition tests, `@omniwa/infrastructure-provider-bridge` bridge contract/fake tests, and a production Docker template that includes API, worker, webhook dispatcher, provider runtime, background, PostgreSQL, and Redis services.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Provider command bridge runtime wiring, worker/provider true production profiles, and target-environment orchestration evidence are not complete.                                 |
...
- `ADR-0010` is accepted, and `@omniwa/infrastructure-provider-bridge` now provides the internal
  ...
  tests without importing Baileys. Worker/provider runtime bridge wiring and production profile
  enablement remain open follow-up work.
