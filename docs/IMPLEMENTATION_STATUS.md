...
- N11 queue hardening now has local PostgreSQL proof for single-reservation concurrency and durable
  retry visibility across queue-provider restarts with
...
- N11.1 adds a durable WorkerJob-backed queue provider. PostgreSQL-backed atomic reservation and
  durable retry visibility now have local contract coverage; lease expiry semantics and final
  production queue validation remain open hardening work.
