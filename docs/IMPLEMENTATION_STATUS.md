...
- N11.5 authorization and rate limits is active. API runtime composition can now wire the existing
  ...
  evidence through `OMNIWA_API_SECURITY_AUDIT_IN_MEMORY=true`. Distributed rate limiting, persistent
  ownership resolution, persistent security-audit evidence, and metrics export remain open N11.5
  follow-up work.
