...
| Runtime apps                                                                                     | Frozen                      | API, worker, webhook dispatcher, provider runtime, background, scheduler, health, metrics, and projection-builder apps exist | `apps/*/src`, runtime composition tests, and a production Docker template that includes API, worker, webhook dispatcher, provider runtime, PostgreSQL, and Redis services.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Multi-process socket bridge, worker/provider true production profiles, and target-environment orchestration evidence are not complete.                                            |
| Webhooks / Events / Realtime                                                                     | Frozen                      | Durable EventLog/replay and webhook dispatcher reliability foundations present                                               | Webhook dispatcher runtime, durable JSON EventLog/outbox, generic EventOutboxConsumer foundation, PostgreSQL EventLog backend, background outbox runtime loop foundation with scheduled backlog metric recording, SSE read surfaces, restart replay tests, safe provider signal ingestion, retry/dead-letter tests, persisted `WebhookDelivery` status updates for delivered/retrying/dead-letter dispatch outcomes, controlled retry, single-delivery redrive, selected bulk redrive operations, dead-letter filtered operator list contract with safe remediation reason/category filters, durable worker-job webhook queue profile, HMAC/timestamp signing, replay verification, `FetchWebhookHttpGateway` timeout/network failure mapping, and opt-in local dispatcher runtime fetch gateway wiring through env. | Production downstream outbox publisher composition, production scaling, target-environment proof, dashboards, and alerting need further hardening.                                |
...
- N11.2 durable EventLog/outbox/SSE replay foundation is present. A generic async-compatible outbox
  ...
  background outbox runtime loop with scheduled backlog metric recording now exist. Production
  downstream publisher composition, target-environment wiring evidence, and alerting remain open
  hardening work.
