...
- N11.6 webhook reliability is active. The dispatcher runtime already has durable restart recovery,
  ...
  `idempotency-key`, synchronized across client-contract fixtures and the Rust SDK. Production
  runtime wiring remains disabled until production queue, secret, HTTP gateway, and observability
  adapters can be composed together safely. Dead-letter redrive beyond the current retryable
  delivery states remains a follow-up operational capability.
