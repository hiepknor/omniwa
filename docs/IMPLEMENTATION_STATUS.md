...
- N11 production compose hardening now expands `deploy/docker/compose.production.yml` beyond the API
  ...
  production profile validation, and explicit controlled-pilot settings for Provider Runtime until
  its production profile and target-environment evidence are complete.
- N11 production compose validation now has a dedicated `pnpm docker:production:check` gate wired
  ...
  PostgreSQL API EventLog backend, Worker production profile wiring, and the intentional Provider
  Runtime controlled-pilot profile without printing rendered secret material.
- N11 Worker production profile validation now fails closed unless Worker uses PostgreSQL
  ...
  and token configuration, and a durable `OMNIWA_EVENT_LOG_PATH`.
...
- Production Docker template coverage is broader than the API service now, but it is still a
  deployment template with Provider Runtime in controlled-pilot profile. Target-environment startup,
  production load, SLO evidence, Provider Runtime production profile, and provider-runtime bridge
  evidence remain open before any production-ready claim.
- `ADR-0010` is accepted, and `@omniwa/infrastructure-provider-bridge` now provides the internal
  ...
  endpoint/token, and a durable EventLog path are configured. The production compose template/check
  now declares Worker production bridge wiring to the Provider Runtime controlled-pilot profile, and
  target-environment runtime evidence now requires sanitized bridge client/server/auth/round-trip
  proof. Provider Runtime production profile enablement and actual target-environment evidence
  collection remain open follow-up work.
- N11.3 added durable local and PostgreSQL provider-runtime ownership lease guards, active lease
  ...
  Provider Runtime production profile enablement remains blocked by final production validation.
...
- Provider-runtime bridge target-environment proof and Provider Runtime production profile
