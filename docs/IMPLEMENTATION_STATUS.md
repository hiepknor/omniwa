...
- N11.5 authorization and rate limits is active. API runtime composition can now wire the existing
  ...
  `OMNIWA_API_SECURITY_AUDIT_RECORDS=true` when the selected repository profile provides
  ...
  resources without current owner fields, and PostgreSQL AuditRecord persistence remain open N11.5
  follow-up work.
