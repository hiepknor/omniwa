...
- N11.6 webhook reliability is active. The dispatcher runtime already has durable restart recovery,
  ...
  `WorkerJob`-backed queue provider. Production runtime wiring remains disabled until production
  queue, secret, HTTP gateway, and observability adapters can be composed together safely. Bulk
  redrive and richer dead-letter remediation remain follow-up operational capabilities.
